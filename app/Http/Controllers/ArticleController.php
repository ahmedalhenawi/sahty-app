...
class ArticleController extends Controller
{
    ...
    public function geDoctorArticles(Request $request){
        ...
        $articles = Article::with('doctor')->where("user_id" , $d_id)->orderBy('created_at', 'desc')->paginate(50);
        return ArticleResource::collection($articles);
    }

    public function getDoctorsArticles(Request $request){
        $articles = Article::with('doctor')->orderBy('created_at', 'desc')->paginate(50);
        return ArticleResource::collection($articles);
    }
    ...
    public function getUserArticles(Request $request){
        ...
        // dd($doctors_id);
        $articles = Article::with('doctor')
                    ->whereIn("user_id"  ,  $doctors_id)
